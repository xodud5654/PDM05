...
loss_function = st.sidebar.selectbox('Loss function', [
                                     'mean_squared_error', 'mean_absolute_error', 'categorical_crossentropy'], key="<uniquevalueofsomesort_1>")
optimizer = st.sidebar.selectbox('Optimizer', ['SGD', 'RMSprop', 'Adam'], key="<uniquevalueofsomesort_2>")
...
if st.checkbox('첫번째 은닉층(hidden layer)에 적용할 활성화함수와, 노드의 갯수를 선택하세요.', key='cb1'):
    fcn1 = st.selectbox(
        "노드의 갯수 :", [32, 64, 128, 256, 512], key="<uniquevalueofsomesort1>")
    act1 = st.selectbox(
        "활성화 함수 목록 :", ['relu',  'tanh', 'sigmoid'], key="<uniquevalueofsomesort1_1>")
    ...
    if st.checkbox('첫번째 은닉층에 dropout를 설정 하시겠습니까?', key='cb11'):
        drop1 = st.selectbox('dropout 비율을 선택해주세요.', [
                             0.0, 0.1, 0.25, 0.5], key="<uniquevalueofsomesort1_2>")
...
if st.checkbox('두번째 은닉층(hidden layer)에 적용할 활성화함수와, 노드의 갯수를 선택하세요.', key='cb2'):
    ...
    act2 = st.selectbox(
        "활성화 함수 목록 :", ['relu',  'tanh', 'sigmoid'], key="<uniquevalueofsomesort2_1>")
    ...
    if st.checkbox('두번째 은닉층에 dropout를 설정 하시겠습니까?', key='cb21'):
        drop2 = st.selectbox('dropout 비율을 선택해주세요.', [
                             0.0, 0.1, 0.25, 0.5], key="<uniquevalueofsomesort2_2>")
...
if st.checkbox('세번째 은닉층(hidden layer)에 적용할 활성화함수와, 노드의 갯수를 선택하세요.', key='cb3'):
    ...
    act3 = st.selectbox(
        "활성화 함수 목록 :", ['relu',  'tanh', 'sigmoid'], key="<uniquevalueofsomesort3_1>")
    ...
    if st.checkbox('세번째 은닉층에 dropout를 설정 하시겠습니까?', key='cb31'):
        drop3 = st.selectbox('dropout 비율을 선택해주세요.', [
                             0.0, 0.1, 0.25, 0.5], key="<uniquevalueofsomesort3_2>")
...
if st.checkbox('네번째 은닉층(hidden layer)에 적용할 활성화함수와, 노드의 갯수를 선택하세요.', key='cb4'):
    ...
    act4 = st.selectbox(
        "활성화 함수 목록 :", ['relu',  'tanh', 'sigmoid'], key="<uniquevalueofsomesort4_1>")
    ...
    if st.checkbox('네번째 은닉층에 dropout를 설정 하시겠습니까?', key='cb41'):
        drop4 = st.selectbox('dropout 비율을 선택해주세요.', [
                             0.0, 0.1, 0.25, 0.5], key="<uniquevalueofsomesort4_2>")
...
fit_len = st.selectbox(
        "학습에 이용할 데이터 수는?", ["1000","all"], key="<uniquevalueofsomesort4_3>")
