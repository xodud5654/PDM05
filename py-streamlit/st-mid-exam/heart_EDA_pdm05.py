...
st.sidebar.subheader('Visualization using seaborn and plotly by pdm05')
...
# def zero2median(df):
#     columns_with_zero = df.columns[df.isna().sum() > 0][1:-1]
...
st.sidebar.write('non-Heart-Disease(noHD):',noHD)
st.sidebar.write('Heart-Disease(HD):',HD)
...
st.write("=> age, cp, thalach, exang, oldpeak, slope, ca, thal ")

df2 = df.copy()
df3 = df2[["age","cp","thalach","exang","oldpeak","slope","ca","thal"]]
fig6 = plt.figure(figsize=(12,10))
sns.heatmap(df3.corr(), annot=True, vmin=-1, vmax=1, cmap='coolwarm')
st.pyplot(fig6)
